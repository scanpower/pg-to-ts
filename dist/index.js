...
function typescriptOfSchema(dbIn, tables, excludedTables, inSchema, options) {
    if (tables === void 0) { tables = []; }
    if (excludedTables === void 0) { excludedTables = []; }
    if (inSchema === void 0) { inSchema = null; }
    if (options === void 0) { options = {}; }
    return __awaiter(this, void 0, void 0, function () {
        var db, schema, optionsObject, enumTypes, _a, interfacePromises, interfaceTuples, tablesToNames, interfaces, typesToImport, _i, _b, types, importTs, symbols, tableNames, typeMaps, tableMap, output, formatterOption, processedResult;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    db = typeof dbIn === 'string' ? new schemaPostgres_1.PostgresDatabase(dbIn) : dbIn;
                    schema = inSchema !== null && inSchema !== void 0 ? inSchema : db.getDefaultSchema();
                ...
                case 4:
                    interfaceTuples = _c.sent();
                    tablesToNames = {};
                    tables.forEach(function (table, i) {
                        tablesToNames[table] = interfaceTuples[i][1];
                    });
                    interfaces = interfaceTuples
                        .map(function (_a) {
                        var ts = _a[0];
                        return (0, typescript_1.attachJoinTypes)(ts, tablesToNames);
                    })
                        .join('');
                    typesToImport = new Set();
                    for (_i = 0, _b = interfaceTuples.map(function (_a) {
                        var types = _a[2];
                        return types;
                    }); _i < _b.length; _i++) {
                        types = _b[_i];
                    ...
                    tableNames = interfaceTuples.map(function (_a) {
                        var names = _a[1];
                        return names;
                    });
                    typeMaps = tableNames
